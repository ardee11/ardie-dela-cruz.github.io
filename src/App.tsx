...
export default function App() {
  ...
  // --- ADDED: State to manage which section is active ---
  const [activeMode, setActiveMode] = useState<'home' | 'about' | 'skills' | 'projects'>('home')
...
const moveCameraAndPose = (x: number, y: number, z: number, mode: 'about' | 'skills' | 'home' | 'projects') => {
  if (!cameraRef.current || !nodes) return
  
  setActiveMode(mode)

  // 1. Kill the background loop
  if (loopRef.current) {
    loopRef.current.kill()
    loopRef.current = null
    
    // Clear the black overlay if it was stuck
    gsap.to(overlayRef.current, { opacity: 0, duration: 0.3 })

    // Handle Hero Text Transition
    const tlText = gsap.timeline();
    tlText.to(textRef.current, {
      left: "100%", xPercent: -100, x: -40, bottom: "20px",
      duration: 1.8, ease: "expo.inOut",
    });
    tlText.to(".hero-name", { fontSize: "1.5rem", fontWeight: 600, duration: 1.8, ease: "expo.inOut" }, 0);
    tlText.to(".hero-sub", { fontSize: "0.5rem", letterSpacing: "0.3em", opacity: 0.8, duration: 1.8, ease: "expo.inOut" }, 0);
  }

  // 2. SAFETY RESET: If camera is currently behind (Z < 0), snap it to front instantly
  ...
  // 3. CAMERA LOGIC (Restoring the Projects Panning)
  if (mode === 'projects') {
    // Projects uses a static lookAt for that "looking up" effect
    cameraRef.current.lookAt(0, 0.2, 0)
    gsap.to(cameraRef.current.position, { x, y, z, duration, ease })
  } else {
    // Others use dynamic onUpdate lookAt for a locked-on feel
  ...
  if (mode === 'about') {
    ...
    gsap.to(nodes.RightArm.rotation, { x: 1, y: -.05, z: -0.7, duration, ease })
    gsap.to(nodes.RightForeArm.rotation, { x: 1.85, duration, ease })
    gsap.to(nodes.Head.rotation, { x: -.32, y: 0.15, z: 0, duration, ease })
  } else if (mode === 'skills') {
  ...
  useEffect(() => {
    ...
    loopRef.current = tl
    const fadeOut = () => tl.to(overlayRef.current, { opacity: 1, duration: 0.5 })
    const fadeIn = () => tl.to(overlayRef.current, { opacity: 0, duration: 0.5 })
    ...
    <div className="main-container">
      <nav className="game-nav">
        <NavButton 
          ...
          onClick={() => moveCameraAndPose(4, -.8, 10, 'about')} 
        />
        <NavButton 
          icon={User} 
          label="Skills" 
      ...
      <div className="canvas-wrapper" style={{ position: 'relative', width: '100vw', height: '100vh', zIndex: 1 }}>
        <div ref={overlayRef} className="scene-overlay" style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', background: '#030303', zIndex: 10, pointerEvents: 'none', opacity: 0 }} />
